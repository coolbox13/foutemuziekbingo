...
def api_get_playlists():
    ...
    try:
        ...
        current_app.logger.error(f"Error loading playlists: {e}")
...
def api_add_playlist():
    ...
    try:
        ...
        return jsonify(
            {"message": "Playlist added successfully", "playlist": new_playlist}
        )
    except Exception as e:
        current_app.logger.error(f"Error adding playlist: {e}")
        return jsonify({"error": str(e)}), 500
...
def api_remove_playlist():
    ...
    try:
        ...
        current_app.logger.error(f"Error removing playlist: {e}")
        return jsonify({"error": str(e)}), 500
...
def api_load_playlist():
    ...
    try:
        ...
        def update_game_state(state):
            # Randomly sample tracks if there are more than 100
            state["unplayed_tracks"] = random.sample(tracks, min(100, len(tracks)))
            state["played_tracks"] = []
            state["cards"] = {}  # Reset cards when loading new playlist
...
def api_set_default_playlist():
    ...
    try:
        ...
        current_app.logger.error(f"Error setting default playlist: {e}")
        return jsonify({"error": str(e)}), 500
