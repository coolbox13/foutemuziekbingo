...
- POST /api/add_playlist: Add a Spotify playlist to user collection
...
from app.validation_models import AddPlaylistRequest
...
@router.post("/api/add_playlist", response_model=Playlist)
async def add_playlist(
    request_data: AddPlaylistRequest,
    request: Request,
    current_user: User = Depends(get_current_user)
):
    """
    Add a Spotify playlist to the user's collection for game creation.

    This endpoint allows users to add playlists from Spotify to their collection,
    making them available for bingo game creation. The playlist is fetched from
    Spotify, validated for game suitability, and cached in the database with
    full track metadata for optimal performance.

    Args:
        request_data (AddPlaylistRequest): Request body containing:
            - playlist_id: Spotify playlist ID to add
            - is_default: Whether to set as default playlist
        request (Request): FastAPI request object containing session data
        current_user (User): Authenticated user from JWT token dependency

    Returns:
        Playlist: The added playlist object with metadata including:
            - spotify_id: Spotify playlist identifier
            - name: Playlist name from Spotify
            - description: Playlist description
            - track_count: Number of tracks in the playlist
            - tracks: Full track listing with metadata
            - created_at: Timestamp when added to user's collection
            - updated_at: Last synchronization timestamp

    Raises:
        HTTPException:
            - 400: Invalid playlist ID format or playlist not accessible
            - 401: Invalid or expired authentication
            - 403: Insufficient Spotify permissions or playlist is private
            - 404: Playlist not found on Spotify
            - 409: Playlist already exists in user's collection
            - 429: Rate limit exceeded
            - 500: Internal server error or Spotify API failure
    ...
    Business Logic:
        1. Validates playlist ID format and accessibility
        2. Fetches playlist metadata from Spotify API
        3. Checks if playlist already exists in user's collection
        4. Loads and caches all track metadata for performance
        5. Stores playlist in database with proper indexing
        6. Optionally sets as user's default playlist
        7. Returns complete playlist object for immediate use

    Performance Notes:
        - Playlist metadata is cached to minimize Spotify API calls
        - Track data is eagerly loaded to ensure game creation readiness
        - Database operations use optimized queries and indexes
        - Concurrent requests are handled safely with proper locking
    """
    try:
        logger.info(
            "[PLAYLIST-ADD-001] Starting add playlist request",
            extra={
                "user_id": current_user.id,
        ...
        # Get Spotify client for API access
        spotify_client = await get_spotify_client(request)
        logger.debug("[PLAYLIST-ADD-002] Spotify client obtained")

        # Check if playlist already exists in user's collection
        existing_playlist = await playlist_service.get_playlist_by_spotify_id(
            request_data.playlist_id, current_user.id
        )

        if existing_playlist:
            logger.warning(
                "[PLAYLIST-ADD-003] Playlist already exists in collection",
                extra={
                    "user_id": current_user.id,
                    "playlist_id": request_data.playlist_id,
                    "existing_id": existing_playlist.id,
                },
            )
            raise HTTPException(
                status_code=409,
                detail="Playlist already exists in your collection"
            )

        # Fetch playlist from Spotify to validate access and get metadata
        try:
            spotify_playlist = spotify_client.playlist(request_data.playlist_id)
            logger.debug(
                "[PLAYLIST-ADD-004] Playlist fetched from Spotify",
                extra={
                    "playlist_name": spotify_playlist.get("name"),
                    "track_count": spotify_playlist.get("tracks", {}).get("total", 0),
                    "owner": spotify_playlist.get("owner", {}).get("display_name"),
                },
            )
        except Exception as e:
            logger.error(
                "[PLAYLIST-ADD-ERROR] Failed to fetch playlist from Spotify",
                extra={
                    "user_id": current_user.id,
                    "playlist_id": request_data.playlist_id,
                    "error": str(e),
            ...
            raise HTTPException(
                status_code=404,
                detail="Playlist not found or not accessible"
            )

        # Validate playlist is suitable for games (has sufficient tracks)
        track_count = spotify_playlist.get("tracks", {}).get("total", 0)
        if track_count < 25:
            logger.warning(
                "[PLAYLIST-ADD-005] Playlist has insufficient tracks for games",
                extra={
                    "user_id": current_user.id,
                    "playlist_id": request_data.playlist_id,
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Playlist must have at least 25 tracks for bingo games (found {track_count})"
            )
        ...
        created_playlist = await playlist_service._create_cached_playlist(
            current_user, spotify_playlist
        )

        logger.info(
            "[PLAYLIST-ADD-006] Playlist successfully added to collection",
            extra={
                "user_id": current_user.id,
                "playlist_id": request_data.playlist_id,
                "playlist_name": created_playlist.name,
                "track_count": created_playlist.total_tracks,
                "database_id": created_playlist.id,
            },
        )

        # Load tracks immediately for better user experience
        try:
            tracks = await playlist_service.load_playlist_tracks_from_spotify(
                spotify_client, request_data.playlist_id
            )
            ...
            logger.warning(
                "[PLAYLIST-ADD-WARNING] Failed to preload tracks, will load on demand",
                extra={
                    "playlist_id": created_playlist.id,
                    "error": str(e),
                },
            )
            # Don't fail the request if track loading fails - tracks can be loaded later

        # Handle default playlist setting if requested
        if request_data.is_default:
            # Note: Default playlist handling would be implemented here
            # For now, we'll just log the intent
            logger.info(
                "[PLAYLIST-ADD-008] Playlist marked as default",
                extra={
                    "user_id": current_user.id,
                    "playlist_id": created_playlist.id,
                },
        ...
        logger.error(
            "[PLAYLIST-ADD-ERROR] Playlist service error",
            extra={
                "user_id": current_user.id,
                "playlist_id": request_data.playlist_id,
                "error": e.message,
        ...
        logger.error(
            "[PLAYLIST-ADD-ERROR] Unexpected error adding playlist",
            extra={
                "user_id": current_user.id,
                "playlist_id": request_data.playlist_id,
                "error": str(e),
