...
async def spotify_api_call_with_retry(
    ...
    for attempt in range(max_retries + 1):
        try:
            logger.debug(f"[SPOTIFY-API] Attempting {operation_name}", extra={
                "attempt": attempt + 1,
            ...
            if attempt > 0:
                logger.info(f"[SPOTIFY-API] {operation_name} succeeded after {attempt} retries")
            ...
            logger.warning(f"[SPOTIFY-API] {operation_name} failed", extra={
                "attempt": attempt + 1,
            ...
            if status_code == 429 and attempt < max_retries:
                ...
                if retry_after:
                    ...
                    logger.info(f"[SPOTIFY-API] Rate limited, waiting {wait_time}s as suggested by Spotify")
                else:
                    wait_time = base_delay * (2 ** attempt) + random.uniform(0, 1)
                    logger.info(f"[SPOTIFY-API] Rate limited, waiting {wait_time:.2f}s (exponential backoff)")
            ...
            user_message = SPOTIFY_ERROR_MESSAGES.get(status_code, "An error occurred with Spotify.")
            logger.error(f"[SPOTIFY-API] {operation_name} failed permanently", extra={
                "status_code": status_code,
                "error": str(e),
            ...
            last_exception = e
            logger.error(f"[SPOTIFY-API] Unexpected error in {operation_name}", extra={
                "attempt": attempt + 1,
                "error": str(e),
            ...
            raise SpotifyAPIError("A technical error occurred. Please try again.", 500, e)
...
async def get_devices_with_cache(sp: spotipy.Spotify, force_refresh: bool = False) -> List[Dict[str, Any]]:
    """
    ...
    if (not force_refresh and 
        _device_cache["last_updated"] and 
        now - _device_cache["last_updated"] < _device_cache["cache_duration"]):
    ...
    try:
        ...
        logger.info(f"[SPOTIFY-DEVICES] Device cache updated", extra={
        ...
        if _device_cache["devices"]:
            logger.warning(f"[SPOTIFY-DEVICES] API failed, using stale cache", extra={
                "error": e.message,
                "cache_age_minutes": (now - _device_cache["last_updated"]).total_seconds() / 60
            })
            return _device_cache["devices"]
...
async def find_active_device_with_fallback(sp: spotipy.Spotify) -> Optional[Dict[str, Any]]:
    """
        ...
        for device_type in preferred_types:
            for device in available_devices:
                if device.get("type") == device_type:
                    logger.info(f"[SPOTIFY-DEVICE] Using fallback device: {device.get('name')} ({device_type})")
                    return device
        
        # If no preferred type found, use the first available
        device = available_devices[0]
        logger.info(f"[SPOTIFY-DEVICE] Using first available device: {device.get('name')}")
        return device
    
    # Fallback 2: Try to activate the first device we can find
    if devices:
        device = devices[0]
        logger.info(f"[SPOTIFY-DEVICE] Attempting to activate device: {device.get('name')}")
        ...
        try:
            ...
            logger.warning(f"[SPOTIFY-DEVICE] Failed to activate device", extra={
...
async def play_track_with_fallback(
    ...
    if device_id:
        ...
        if not target_device:
            logger.warning(f"[SPOTIFY-PLAYBACK] Specified device not found: {device_id}")
    ...
    try:
        ...
        logger.info(f"[SPOTIFY-PLAYBACK] Track playing successfully", extra={
            "track_uri": track_uri,
        ...
        if "device" in e.message.lower():
            logger.info("[SPOTIFY-PLAYBACK] Device error, refreshing device list and retrying")
            ...
            if backup_device and backup_device["id"] != target_device["id"]:
                try:
                    ...
                    logger.info(f"[SPOTIFY-PLAYBACK] Retry successful with backup device", extra={
                        "backup_device": backup_device.get("name")
                    })
...
async def pause_playback_safe(sp: spotipy.Spotify, device_id: Optional[str] = None) -> bool:
    """
...
def convert_spotify_exception_to_http(e: Exception, operation: str = "Spotify operation") -> HTTPException:
    """
    ...
    if isinstance(e, SpotifyAPIError):
        ...
        logger.error(f"[SPOTIFY-ERROR] {operation} failed", extra={
            "status_code": status_code,
            "error": str(e)
        })
        
        return HTTPException(status_code=400 if status_code < 500 else 500, detail=user_message)
    
    else:
        # Generic error - don't expose internal details
        logger.error(f"[SPOTIFY-ERROR] Unexpected error in {operation}", extra={
            "error": str(e),
            "error_type": type(e).__name__
        })
