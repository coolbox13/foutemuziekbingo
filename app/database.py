...
class SupabaseService:
    ...
    async def initialize(self) -> None:
        ...
        try:
            logger.info(f"[DB-INIT-001] Starting database initialization", extra={
                "init_id": init_id,
            ...
            if not supabase_url or not supabase_service_key:
                ...
                logger.error(f"[DB-INIT-ERROR-001] Missing required Supabase configuration", extra={
                    "init_id": init_id,
            ...
            logger.info(f"[DB-INIT-002] Creating Supabase client", extra={
                "init_id": init_id,
            ...
            logger.info(f"[DB-INIT-003] Testing database connection", extra={"init_id": init_id})
            ...
            if health_check_result["is_healthy"]:
                ...
                logger.info(f"[DB-INIT-004] Database connection established successfully", extra={
                    "init_id": init_id,
                ...
                logger.warning(f"[DB-INIT-WARN-004] Database connection has issues", extra={
                    "init_id": init_id,
                    "health_check": health_check_result
                })
                ...
                if os.getenv("NODE_ENV") == "development":
                    logger.warning(f"[DB-INIT-WARN-005] Continuing in development mode despite database connection issues", extra={"init_id": init_id})
                    self.is_initialized = True
                else:
                    raise DatabaseError("Database connection health check failed")

            logger.info(f"[DB-INIT-005] Database initialization completed", extra={
                "init_id": init_id,
            ...
            logger.error(f"[DB-INIT-ERROR] Database initialization failed", extra={
                "init_id": init_id,
                "error": {
                    "message": str(error),
            ...
            if os.getenv("NODE_ENV") == "development":
                logger.warning(f"[DB-INIT-WARN] Continuing in development mode despite initialization failure", extra={"init_id": init_id})
                self.is_initialized = True
            else:
    ...
    async def perform_health_check(self) -> Dict[str, Any]:
        ...
        logger.debug(f"[DB-HEALTH-001] Starting database health check", extra={"health_check_id": health_check_id})
        ...
        try:
            ...
            if result["checks"]["connection"]:
                ...
                try:
                    response = self.client.table("users").select("*", count="exact").limit(0).execute()
                    result["checks"]["auth"] = True
                    logger.debug(f"[DB-HEALTH-004] Auth check result", extra={
                ...
                for table in critical_tables:
                    try:
                        ...
                        logger.debug(f"[DB-HEALTH-005] Table check result", extra={
                            "health_check_id": health_check_id,
                            "table": table,
                        ...
                        logger.debug(f"[DB-HEALTH-WARN-005] Table check failed", extra={
                            "health_check_id": health_check_id,
                            "table": table,
            ...
            result["is_healthy"] = result["checks"]["connection"] and result["checks"]["auth"]
        ...
        result["response_time"] = (end_time - start_time).total_seconds() * 1000  # Convert to milliseconds
        self.last_health_check = result["timestamp"]

        logger.debug(f"[DB-HEALTH-006] Health check completed", extra={
            "health_check_id": health_check_id,
            "result": {
    ...
    async def execute_query(self, query_fn):
        ...
        try:
            ...
            logger.error(f"[DB-QUERY-ERROR] Unexpected database error", extra={
                "error": {
                    "message": str(error),
                    "type": type(error).__name__
                }
    ...
    async def create_record(self, table: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            logger.debug(f"[DB-CREATE] Creating record", extra={"table": table, "data_keys": list(data.keys())})
            ...
            logger.error(f"[DB-CREATE-ERROR] Create record failed", extra={
                "table": table,
                "error": str(error)
            })
            raise DatabaseError(message, error)
    ...
    async def get_record(self, table: str, record_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            logger.debug(f"[DB-GET] Getting record", extra={"table": table, "id": record_id})
            ...
            if response.data and len(response.data) > 0:
                ...
                logger.debug(f"[DB-GET] Record not found", extra={"table": table, "id": record_id})
                return None
            ...
            logger.error(f"[DB-GET-ERROR] Unexpected get record error", extra={
                "table": table,
                "id": record_id,
                "error": str(error)
            })
    ...
    async def update_record(self, table: str, record_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
        """Update a record by ID"""
        try:
            logger.debug(f"[DB-UPDATE] Updating record", extra={"table": table, "id": record_id, "update_keys": list(updates.keys())})
            
            def query_fn(client):
                return client.table(table).update(updates).eq("id", record_id).execute()
            ...
            logger.error(f"[DB-UPDATE-ERROR] Update record failed", extra={
                "table": table,
                "id": record_id,
                "error": str(error)
            })
    ...
    async def delete_record(self, table: str, record_id: str) -> None:
        ...
        try:
            logger.debug(f"[DB-DELETE] Deleting record", extra={"table": table, "id": record_id})
            ...
            logger.debug(f"[DB-DELETE] Record deleted successfully", extra={"table": table, "id": record_id})
            
        except Exception as error:
            if isinstance(error, DatabaseError):
                raise error
            message = f"Unexpected error deleting record from {table}: {str(error)}"
            logger.error(f"[DB-DELETE-ERROR] Unexpected delete record error", extra={
                "table": table,
                "id": record_id,
                "error": str(error)
            })
    ...
    async def query_records(self, table: str, **kwargs) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"[DB-QUERY] Querying records", extra={
                "table": table,
            ...
            logger.debug(f"[DB-QUERY] Query completed", extra={
                "table": table,
            ...
            logger.error(f"[DB-QUERY-ERROR] Unexpected query records error", extra={
                "table": table,
                "error": str(error)
            })
            raise DatabaseError(message, error)
    ...
    async def count_records(self, table: str, filters: Optional[Dict[str, Any]] = None) -> int:
        ...
        try:
            logger.debug(f"[DB-COUNT] Counting records", extra={
                "table": table,
                "filter_count": len(filters) if filters else 0
            })
            ...
            logger.debug(f"[DB-COUNT] Count completed", extra={"table": table, "count": result})
            ...
            logger.error(f"[DB-COUNT-ERROR] Unexpected count records error", extra={
                "table": table,
                "error": str(error)
            })
            raise DatabaseError(message, error)
