...
async def api_get_devices(
    ...
    try:
        logger.info("[DEVICE-API-001] Getting devices for user", extra={
        ...
        logger.info("[DEVICE-API-002] Devices retrieved successfully", extra={
            "device_count": len(devices) if devices else 0,
            "active_devices": len([d for d in devices if d.get("is_active")]) if devices else 0
        })
        ...
        logger.error("[DEVICE-API-ERROR] Unexpected error getting devices", extra={
            "user_id": current_user.id,
            "error": str(e),
        ...
        raise HTTPException(
            status_code=500, 
            detail="Unable to retrieve devices. Please try again."
        )
...
async def api_select_device(
    ...
    try:
        ...
        logger.info("[DEVICE-SELECT-001] Selecting device", extra={
            "device_id": device_id,
            "user_id": current_user.id
        })
        ...
        logger.info("[DEVICE-SELECT-002] Device selected successfully", extra={
            "device_id": device_id,
            "user_id": current_user.id
        })
        ...
        logger.error("[DEVICE-SELECT-ERROR] Unexpected error selecting device", extra={
            "error": str(e),
        ...
        raise HTTPException(
            status_code=500, 
            detail="Unable to select device. Please try again."
        )
