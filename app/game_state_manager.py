...
class RedisGameStateManager:
    """
    Production-grade Redis game state manager with:
    ...
    def __init__(self, redis_url: str = None, redis_password: str = None):
        """
        Initialize Redis game state manager.
        ...
        self.redis_password = redis_password or self.config.REDIS_PASSWORD
    ...
    async def initialize(self) -> None:
        ...
        try:
            logger.info("[REDIS-STATE-INIT] Initializing Redis game state manager")
            ...
            logger.info("[REDIS-STATE-INIT] Redis game state manager initialized successfully")
            
        except Exception as error:
            logger.error(f"[REDIS-STATE-INIT-ERROR] Failed to initialize Redis connection: {error}")
            self.is_connected = False
            if self.connection_retries < self.max_retries:
                self.connection_retries += 1
                logger.info(f"[REDIS-STATE-RETRY] Retrying connection in 5 seconds (attempt {self.connection_retries}/{self.max_retries})")
                await asyncio.sleep(5)
                await self.initialize()
            else:
                raise Exception(f"Failed to connect to Redis after {self.max_retries} attempts: {error}")
