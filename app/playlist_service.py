...
class PlaylistService:
    ...
    async def get_user_playlists(self, user: User, spotify_client: Spotify) -> List[Playlist]:
        """Get user's playlists with intelligent caching"""
        logger.info(f"[PLAYLIST-GET-001] Getting playlists for user", extra={
            "user_id": user.id,
        ...
        try:
            ...
            logger.debug(f"[PLAYLIST-GET-002] Found cached playlists", extra={
                "user_id": user.id,
                "cached_count": len(cached_playlists)
            })
            
            # Get fresh playlists from Spotify
            spotify_playlists = await self._fetch_spotify_playlists(spotify_client, user)
            ...
            logger.info(f"[PLAYLIST-GET-003] Playlists retrieved successfully", extra={
                "user_id": user.id,
            ...
            logger.error(f"[PLAYLIST-GET-ERROR] Error getting user playlists", extra={
                "user_id": user.id,
                "error": str(e)
            })
            raise PlaylistError(f"Failed to get playlists: {str(e)}")
    
    async def get_playlist_by_id(self, playlist_id: str, include_tracks: bool = True) -> Optional[Playlist]:
        """Get specific playlist by ID"""
        try:
            logger.debug(f"[PLAYLIST-SINGLE-001] Getting playlist by ID", extra={
            ...
            logger.debug(f"[PLAYLIST-SINGLE-002] Playlist retrieved successfully", extra={
                "playlist_id": playlist_id,
            ...
            logger.error(f"[PLAYLIST-SINGLE-ERROR] Error getting playlist", extra={
                "playlist_id": playlist_id,
                "error": str(e)
            })
            return None
    
    async def get_playlist_by_spotify_id(self, spotify_id: str, owner_id: str) -> Optional[Playlist]:
        """Get playlist by Spotify ID and owner"""
        try:
            playlists = await database.query_records(
                "playlists",
                filters={"spotify_id": spotify_id, "owner_id": owner_id}
            )
            ...
            logger.error(f"[PLAYLIST-SPOTIFY-ERROR] Error getting playlist by Spotify ID", extra={
    ...
    async def _fetch_spotify_playlists(self, spotify_client: Spotify, user: User) -> List[Dict[str, Any]]:
        """Fetch playlists from Spotify API"""
        logger.debug(f"[SPOTIFY-FETCH-001] Fetching playlists from Spotify API", extra={
            "user_id": user.id
        })
        ...
        try:
            ...
            while results:
                for playlist in results['items']:
                    if playlist and playlist.get('id'):
                        ...
                        if (playlist['owner']['id'] == user.spotify_id or 
            ...
            logger.debug(f"[SPOTIFY-FETCH-002] Fetched playlists from Spotify", extra={
                "user_id": user.id,
                "playlist_count": len(spotify_playlists)
            })
            
            return spotify_playlists
            
        except Exception as e:
            logger.error(f"[SPOTIFY-FETCH-ERROR] Error fetching from Spotify API", extra={
                "user_id": user.id,
                "error": str(e)
            })
            raise PlaylistError(f"Failed to fetch playlists from Spotify: {str(e)}")
    
    async def _sync_playlists_cache(self, user: User, spotify_playlists: List[Dict[str, Any]], 
                                   cached_playlists: List[Dict[str, Any]]) -> List[Playlist]:
        """Sync Spotify playlists with database cache"""
        logger.debug(f"[PLAYLIST-SYNC-001] Syncing playlist cache", extra={
            "user_id": user.id,
        ...
        try:
            ...
            for spotify_playlist in spotify_playlists:
                ...
                if spotify_id in cached_lookup:
                    ...
                    if needs_update:
                        logger.debug(f"[PLAYLIST-SYNC-002] Updating cached playlist", extra={
                    ...
                    logger.debug(f"[PLAYLIST-SYNC-003] Creating new cached playlist", extra={
                        "spotify_id": spotify_id,
                        "name": spotify_playlist["name"],
                        "user_id": user.id
                    })
                    
                    new_playlist = await self._create_cached_playlist(user, spotify_playlist)
                    synced_playlists.append(new_playlist)
            
            logger.debug(f"[PLAYLIST-SYNC-004] Playlist sync completed", extra={
                "user_id": user.id,
            ...
            logger.error(f"[PLAYLIST-SYNC-ERROR] Error syncing playlist cache", extra={
                "user_id": user.id,
                "error": str(e)
            })
            raise PlaylistError(f"Failed to sync playlist cache: {str(e)}")
    
    async def _create_cached_playlist(self, user: User, spotify_playlist: Dict[str, Any]) -> Playlist:
        ...
        try:
            ...
            await self._refresh_playlist_tracks(created_playlist["id"], spotify_playlist)
            
            return Playlist(**created_playlist)
            
        except Exception as e:
            logger.error(f"[PLAYLIST-CREATE-ERROR] Error creating cached playlist", extra={
                "spotify_id": spotify_playlist["id"],
                "user_id": user.id,
                "error": str(e)
            })
    ...
    async def _refresh_playlist_tracks(self, playlist_id: str, spotify_playlist: Dict[str, Any]):
        """Refresh tracks for a playlist"""
        logger.debug(f"[PLAYLIST-TRACKS-001] Refreshing playlist tracks", extra={
            "playlist_id": playlist_id,
            "spotify_id": spotify_playlist["id"]
        })
        ...
        try:
            ...
            logger.debug(f"[PLAYLIST-TRACKS-002] Playlist tracks refreshed", extra={
                "playlist_id": playlist_id,
                "track_count": len(tracks_data)
            })
            
        except Exception as e:
            logger.error(f"[PLAYLIST-TRACKS-ERROR] Error refreshing playlist tracks", extra={
                "playlist_id": playlist_id,
                "error": str(e)
            })
            # Don't raise error here - playlist can exist without tracks temporarily
    
    async def load_playlist_tracks_from_spotify(self, spotify_client: Spotify, 
                                               playlist_id: str) -> List[Track]:
        """Load fresh tracks from Spotify API"""
        logger.debug(f"[SPOTIFY-TRACKS-001] Loading tracks from Spotify", extra={
            "playlist_id": playlist_id
        })
        ...
        try:
            ...
            logger.debug(f"[SPOTIFY-TRACKS-002] Loaded tracks from Spotify", extra={
                "playlist_id": playlist_id,
            ...
            logger.error(f"[SPOTIFY-TRACKS-ERROR] Error loading tracks from Spotify", extra={
                "playlist_id": playlist_id,
                "error": str(e)
            })
        ...
        try:
            ...
            for playlist_data in all_playlists:
                ...
                if playlist_data.get("total_tracks", 0) >= 25:
                    playlist = await self.get_playlist_by_id(playlist_data["id"], include_tracks=False)
                    if playlist:
                        suitable_playlists.append(playlist)
            
            logger.debug(f"[PLAYLIST-GAME-001] Found suitable playlists for game", extra={
                "user_id": user.id,
            ...
            logger.error(f"[PLAYLIST-GAME-ERROR] Error getting game playlists", extra={
                "user_id": user.id,
                "error": str(e)
            })
            return []
